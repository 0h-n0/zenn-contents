...
- VRAM使用量・推論速度のベンチマーク情報とチューニング手法
...
RTX 3090単体でQwen3.5-35B-A3B（MoEモデル）をQ4量子化で動かすと、32Kコンテキストで**約80〜110 tok/s**のトークン生成速度が得られます（コンテキスト長が長くなると速度は低下し、131Kでは約79 tok/sまで下がります）。これはバイブコーディングに十分な応答速度です。SWE-bench Verifiedスコアでは、Qwen3.5フルモデル（397B-A17B）が76.4%を記録しており、Claude Sonnet 4.5の77.2%に迫る水準です。ただし、RTX 3090に収まる35B-A3Bモデルではフルモデルより性能が下がるため、**「Sonnet 4.5相当」を1枚のGPUで完全に再現するのは現時点では困難**です。実用上は、定型的なコード生成やリファクタリングでは十分な品質が得られ、複雑なアーキテクチャ設計にはAPIモデルとの併用が有効です。
...
|--------|------------|-------------------|--------------------------|---------|------|
| Qwen3.5-35B-A3B | 35B | 3B（MoE） | 約19GB | 約80-110 tok/s | バイブコーディングに推奨 |
| Qwen3.5-27B | 27B | 27B（Dense） | 約16GB | 約31 tok/s | 高精度だが低速 |
| Qwen3-Coder-Next | 80B | 3B（MoE、512中10エキスパート活性） | 約46GB（GPU+RAM分散） | 約25-40 tok/s | コーディング特化だがRAM必要 |
...
- Qwen3.5世代はマルチモーダル対応（テキスト+画像+動画入力）で、コード中の図表やUI画像の理解にも活用できます
...
| `--temp` | 0.7 | Qwen公式推奨値（Instruct/非思考モード用） |
| `--top-p` | 0.8 | Nucleus samplingの確率閾値（非思考モード用） |

**注意**: Qwen3.5は「思考モード」（デフォルト、temp=1.0, top_p=0.95）と「非思考（Instruct）モード」（temp=0.7, top_p=0.8）の2つの推論モードを持ちます。上記の設定は非思考モードのパラメータです。バイブコーディングでは応答速度を重視するため、非思考モードを推奨します。思考モードを使いたい場合は`--temp 1.0 --top-p 0.95`に変更してください。
...
> **注意**: Continue.devは`config.json`を非推奨とし、YAML形式の`config.yaml`を推奨しています（2026年2月時点）。
...
| ベンチマーク | Qwen3.5-35B-A3B | Qwen3.5-397B-A17B（フル） | Claude Sonnet 4.5 | 備考 |
|------------|-----------------|---------------------|-------------------|------|
| SWE-bench Verified | 69.2% | 76.4% | 77.2% | 実際のGitHub Issue修正タスク |
| HumanEval | 公式データなし | 高水準 | 高水準 | 基本的なコード生成 |
| 推論速度 | 約80-110 tok/s（ローカル） | API依存 | API依存 | RTX 3090 Q4量子化・32Kコンテキスト時 |
| コスト | 電気代のみ（約$0.05/h） | API課金 | API課金 | 長期利用で有利 |

SWE-bench Verifiedのスコアでは、Qwen3.5フルモデル（397B-A17B）がClaude Sonnet 4.5と1ポイント差の76.4%を記録しています。RTX 3090で動く35B-A3Bモデルは69.2%とフルモデルより約7ポイント低く、**複雑なバグ修正やアーキテクチャレベルの設計ではAPIモデルとの差が出ます**。
...
- RTX 3090でQwen3.5-35B-A3B（MoE、Q4量子化）を動かすと、32Kコンテキストで**約80〜110 tok/sの生成速度**が得られ、バイブコーディングが実用的に行えます
- Aider・Continue.dev・Qwen Code CLIの3ツールを使い分けることで、ターミナル・VSCode・エージェント型の多様なワークフローに対応できます
- SWE-benchスコアではQwen3.5フルモデル（397B-A17B）がClaude Sonnet 4.5に迫る水準（76.4% vs 77.2%）ですが、RTX 3090に収まる35B-A3Bモデルでは性能差が広がります
...
本記事に関連する1次情報（論文・テックブログ）の解説記事です。

- [論文解説: DeepSeek-V2 — MoE+MLAアーキテクチャによるKVキャッシュ93.3%削減](https://0h-n0.github.io/posts/paper-2401-06066/) — Qwen3.5が採用するMoEアーキテクチャの理論的基盤
- [論文解説: PowerInfer — コンシューマGPUでのLLM高速推論](https://0h-n0.github.io/posts/paper-2312-11514/) — Hot/Coldニューロン分離によるGPU-CPUハイブリッド推論
- [論文解説: Qwen2.5-Coder Technical Report — コード生成特化LLMの設計と学習](https://0h-n0.github.io/posts/paper-2409-12186/) — Qwen Coderシリーズの技術的詳細
- [テックブログ解説: NVIDIA llama.cpp CUDA Graphs — カーネル起動オーバーヘッド削減](https://0h-n0.github.io/posts/techblog-nvidia-llama-cpp-cuda-graphs/) — llama.cppのGPU推論最適化手法
- [論文解説: NeurIPS 2024 MoE推論最適化 — Dynamic Gating・Expert Buffering](https://0h-n0.github.io/posts/conf-neurips2024-moe-inference/) — MoE推論のスループットを最大11.55倍改善する手法
