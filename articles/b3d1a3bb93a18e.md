...
- vLLM v0.17系（2026年3月時点最新）でのドラフトモデル方式・Eagle3・Medusa Headの設定方法と使い分け
...
ここで $c$ はドラフトモデルの1回のフォワードパスにかかる時間をターゲットモデルの1回のフォワードパスの時間で割った比率です。この式は受理率 $\alpha = 1$（全トークンが受理される理想ケース）での上限を示しており、$c = 0.1$、$\gamma = 5$ で理論上限は4.0倍になります。実際には受理率 $\alpha$ が1未満のため、$\alpha = 0.8$ 程度の場合で約2〜3倍のスピードアップが期待されます。
...
| **学習コスト** | なし（既存モデルを使用） | 必要（Speculators v0.4.0で学習可） | 必要（Medusa HeadのFine-tuning） |
| **報告されている速度向上** | 1.3〜1.5倍 | 1.6〜2.5倍 | 2.2〜3.6倍 |
...
- Eagle3は2026年3月時点でもSOTA（State of the Art）とされており、vLLM v0.8.5以降で公式サポートされています
...
Eagle3は2026年3月時点でも投機的デコーディングのSOTAアルゴリズムとされています。ターゲットモデルの中間隠れ状態（hidden states）を3層から取得し、ドラフトモデルの入力として活用することで高い受理率を実現します。
...
- **Medusa-2**: ベースモデルと追加ヘッドを同時にFine-tuningします。より高い受理率（2.3〜3.6倍の速度向上）が得られますが、ベースモデルの性能を維持するための特別な学習レシピが必要です
...
vLLMでは、投機的デコーディングのメトリクスが組み込みで提供されています（v0.9.1以降）。Prometheusエンドポイント（`/metrics`）から以下のメトリクスを取得できます。
...
| Eagle3でエラー | vLLMのバージョンが古い | v0.17系にアップデート（Eagle3はv0.8.5で導入）。`method`を`"eagle3"`に設定しているか確認 |
...
- vLLMではドラフトモデル方式（1.3〜1.5倍）、Eagle3（1.6〜2.5倍）、Medusa Head（2.2〜3.6倍）の3手法が利用可能です
...
- Speculators（2026年3月時点の最新はv0.4.0）を使って、自社モデル用のEagle3ドラフトモデルを学習し、受理率をさらに向上させる

## 参考

- [vLLM公式ドキュメント: Speculative Decoding](https://docs.vllm.ai/en/latest/features/speculative_decoding/)
- [vLLM Blog: How Speculative Decoding Boosts vLLM Performance](https://vllm.ai/blog/spec-decode)
- [vLLM Blog: Speculators v0.3.0](https://vllm.ai/blog/speculators-v030)
...
- [論文解説: EAGLE — Feature Uncertaintyの再考によるLLM投機的デコーディングの高速化](https://0h-n0.github.io/posts/paper-2401-10774/) - arXiv解説
- [論文解説: Medusa — 複数デコーディングヘッドによるLLM推論高速化フレームワーク](https://0h-n0.github.io/posts/paper-2310-07177/) - arXiv解説
- [論文解説: EAGLE-3 — 学習時テストスケーリングによるLLM投機的デコーディングの進化](https://0h-n0.github.io/posts/paper-2501-05370/) - arXiv解説
- [論文解説: Fast Inference from Transformers via Speculative Decoding — 原論文](https://0h-n0.github.io/posts/paper-2211-17192/) - arXiv解説
- [vLLM公式ブログ解説: 投機的デコーディングの実装と性能評価](https://0h-n0.github.io/posts/techblog-vllm-spec-decode/) - tech_blog解説
