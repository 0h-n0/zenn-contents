...
- Muon/MuonClipオプティマイザがAdamW比**約2倍の計算効率**を達成した仕組み
...
2026年現在、フロンティアLLMの学習は**3段階パイプライン**（事前学習 → 中間学習 → Post-training）に標準化されています。Muon系オプティマイザ（MuonClip含む）の導入で計算効率が約2倍に、MoEアーキテクチャの高スパース化で推論コストが60%以上削減されました。本記事では、7つの最新オープンウェイトモデルのテクニカルレポートを横断比較し、実践で再現可能な学習プレイブックを整理します。
...
|--------|-----------|---------|--------|--------------|
| Kimi K2 | 1.04T（活性32B） | MLA | MoE（384専門家） | MuonClip |
...
2026年の最大の変化は**Muonオプティマイザ**の実用化です。Kimi K2ではMuonにQK-Clip機構を組み合わせた**MuonClip**として大規模に採用され、15.5Tトークンの学習を損失スパイクなしで完走しました。Muon系オプティマイザはAdamW比で**約2倍の計算効率**を実証しています。
...
学習率はKimi K2の例で**WSD（Warmup-Stable-Decay）** を採用しています。500ステップのウォームアップ → 10Tトークンで2e-4定数 → 5.5Tでコサイン減衰（2e-5まで）です。安定期を長く取ることで中間チェックポイントの品質が安定します。
...
- **オプティマイザ**: Muon/MuonClipがAdamW比2倍の効率を実現。ただしEmbedding/HeadにはAdamWを併用
...
- [論文解説: Muon is Scalable for LLM Training — Newton-Schulz直交化による2倍効率のオプティマイザ](https://0h-n0.github.io/posts/paper-2502-16982/) - arxiv解説
- [論文解説: Kimi K2 — 1兆パラメータMoE+MuonClipで実現するオープンエージェント知能](https://0h-n0.github.io/posts/paper-2507-20534/) - arxiv解説
- [論文解説: DeepSeekMath — GRPOによるLLM数学推論の限界突破](https://0h-n0.github.io/posts/paper-2402-03300/) - arxiv解説
- [論文解説: MegaScale-MoE — 1,440 GPU上で1.88倍高速化を実現するMoE学習システム](https://0h-n0.github.io/posts/paper-2505-11432/) - arxiv解説
- [論文解説: Muon + MLA + MoE — 3技術統合で68%メモリ削減・3.2倍推論高速化を実現](https://0h-n0.github.io/posts/paper-2509-24406/) - arxiv解説
