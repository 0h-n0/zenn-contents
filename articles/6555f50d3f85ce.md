...
**注意点:**
> `concurrency_limit`パラメータで同時実行Component数を制限できます。GPUメモリやAPI Rate Limitの制約がある場合は、`pipeline.run_async(data, concurrency_limit=4)`のように上限を設定してください（デフォルト値は4）。制限なしで大量のAPI呼び出しComponentを並列実行すると、429エラーの原因になります。
...
services:
  ...
  jaeger:
    image: jaegertracing/jaeger:2.16
    ports:
      - "16686:16686"
      - "4317:4317"
    command:
...
# ※ @super_component デコレータを使う方法も推奨されている
...
| 429 Too Many Requests | 並列API呼び出しの過多 | `run_async(data, concurrency_limit=N)`で制限 |
...
- [論文解説: RAG for LLMs Survey — RAGパイプライン設計の体系的整理](https://0h-n0.github.io/posts/paper-2312-10997/) - arXiv論文解説
- [論文解説: RAGBench — 説明可能な4次元メトリクスによるRAG評価ベンチマーク](https://0h-n0.github.io/posts/paper-2407-11005/) - arXiv論文解説
- [Haystack公式ベンチマーク解説: RAGパイプラインのパラメータチューニング](https://0h-n0.github.io/posts/techblog-benchmarking-haystack-pipelines/) - tech_blog解説
- [OpenTelemetry公式ブログ解説: LLMアプリケーションの可観測性設計パターン](https://0h-n0.github.io/posts/techblog-otel-llm-observability/) - tech_blog解説
- [OpenTelemetry公式ブログ解説: AIエージェント可観測性の標準化と計装パターン](https://0h-n0.github.io/posts/techblog-otel-agent-observability/) - tech_blog解説
