...
def sparse_search(
    ...
    q_emb = model.encode_query([query])[0]
    # スパーステンソルの行列積でスコア計算
    doc_matrix = torch.stack(doc_embeddings)
    scores_tensor = torch.sparse.mm(doc_matrix, q_emb.unsqueeze(1)).squeeze()
    scored = [(idx, scores_tensor[idx].item()) for idx in range(len(doc_embeddings))]
    # スコア上位を返す（内積が高いほど関連度が高い）
        score = sum(
...
- [論文解説: Matryoshka Representation Learning — 可変次元Embeddingで検索コストを14倍削減](https://0h-n0.github.io/posts/paper-2205-13147/) - arxiv解説
- [論文解説: ColBERTv2 — Residual Compressionで実現する軽量Late Interaction検索](https://0h-n0.github.io/posts/paper-2112-01488/) - arxiv解説
- [論文解説: HyDE — LLM生成の仮想ドキュメントでゼロショット検索精度を改善](https://0h-n0.github.io/posts/paper-2212-10496/) - arxiv解説
- [論文解説: SPLADE v2 — 学習型スパース検索で転置インデックスにセマンティック拡張を導入](https://0h-n0.github.io/posts/paper-2109-10086/) - arxiv解説
- [論文解説: Mistral-SPLADE — LLMのEcho EmbeddingでBEIRスパース検索SOTAを達成](https://0h-n0.github.io/posts/paper-2408-11119/) - arxiv解説
