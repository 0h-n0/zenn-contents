...
- 単一APIキーのレートリミット（OpenAIはTierに応じてRPM制限あり）を回避できる
...
# 高優先度用: フォールバック戦略（Claude Sonnet 4 → GPT-4o）
premium_config = {
    "strategy": {
        "mode": "fallback",
...
# 通常用: ロードバランシング戦略（gpt-4o-mini 70% / gemini-2.5-flash 30%）
standard_config = {
    "strategy": {
        "mode": "loadbalance"
    },
...
def get_client(priority: str) -> Portkey:
    """メタデータの優先度に応じてconfigを切り替える"""
    config = premium_config if priority == "high" else standard_config
    return Portkey(api_key="your_portkey_api_key", config=config)

# 高優先度リクエスト → Claude Sonnet 4（障害時はGPT-4oにフォールバック）
high_priority = get_client("high").chat.completions.create(
    messages=[{"role": "user", "content": "重要な契約書をレビューしてください"}]
)

# 通常リクエスト → gpt-4o-mini(70%) / gemini-2.5-flash(30%) でロードバランシング
standard = get_client("standard").chat.completions.create(
...
|------------|-----------|------------------------|-------------|
| 複雑な推論 | Claude Sonnet 4 | $3.00/1M tokens | 契約書レビュー、複雑なコード生成 |
| 複雑な推論 | GPT-4o | $2.50/1M tokens | 契約書レビュー、複雑なコード生成 |
| 一般的なQ&A | GPT-4o-mini | $0.15/1M tokens | FAQ応答、簡単な要約 |
| 大量バッチ処理 | Gemini 2.5 Flash | $0.30/1M tokens | データ分類、テンプレート生成 |

上記の単価（入力トークン）はOpenAI・Anthropic・Google公式サイトに掲載されている2026年3月時点の公開価格です。タスクの振り分けには前述の条件付きルーティングを使用します。
...
- [Portkey公式料金ページ](https://portkey.ai/pricing)
- [Portkey料金ガイド2026（TrueFoundry解説）](https://www.truefoundry.com/blog/portkey-pricing-guide)

---

## 関連する深掘り記事

本記事のテーマに関連する1次情報（論文・企業テックブログ）の深掘り記事です。

- [RouteLLM: LLMルーティングのコスト品質最適化フレームワーク（arXiv 2406.18665）](https://0h-n0.github.io/posts/paper-routellm-2406-18665/)
- [NVIDIA LLMルーティングBlueprint: Triton+Rustによる高性能推論基盤](https://0h-n0.github.io/posts/techblog-nvidia-llm-routing-blueprint/)
- [Hybrid LLM: 複雑度-品質分離学習によるLLMルーティング最適化（arXiv 2407.00066）](https://0h-n0.github.io/posts/paper-hybrid-llm-2407-00066/)
- [AutoMix: 自己検証と近似最適停止によるLLMルーティング（arXiv 2402.14099）](https://0h-n0.github.io/posts/paper-automix-2402-14099/)
- [AWS Bedrock Intelligent Prompt Routing: マネージドLLMルーティングの実装と活用](https://0h-n0.github.io/posts/techblog-aws-bedrock-intelligent-prompt-routing/)
