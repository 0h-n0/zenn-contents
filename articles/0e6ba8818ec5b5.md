...
Kong社のベンチマーク（2025年公開）では、AWS EKSクラスタ上で3つのAIゲートウェイを同一条件で比較しています。なお、現在Portkey Gateway 2.0のプレリリースが公開されており、ベンチマーク時のv1.9.19からパフォーマンスが改善されている可能性があります。
...
```typescript
// portkey-config.ts
// Portkey Node.js SDK v3.x 系での設定例（2026年3月時点）
...
Portkey OSSは公式に「<1msレイテンシ、122KBのフットプリント」と公表しており、Kong社ベンチマーク（12 CPU割り当て）では約10,300 RPSを記録しています。本番環境での同時接続を安定させるためには、以下の設計パターンが有効です。

```python
# load_balance_config.py
# Portkey Python SDK v2.x 系での重み付きロードバランシング設定（2026年3月時点）
...
# キャッシュ設定はconfigオブジェクトで制御（公式ドキュメント推奨）
cache_config = {
    "cache": {"mode": "semantic", "max_age": 604800},  # 7日間キャッシュ
...
Portkey社の公開情報によると、インテリジェントキャッシュを活用した企業でAPI支出を25%以上削減した事例があると報告されています。ただし、具体的な企業名や詳細な検証条件は公開されていないため、自環境での効果は実測で確認することを推奨します。
...
- [Portkey Cache (Simple & Semantic) - 公式ドキュメント](https://portkey.ai/docs/product/ai-gateway/cache-simple-and-semantic)
- [Portkey Python SDK (PyPI)](https://pypi.org/project/portkey-ai/)
- [Portkey Node.js SDK (GitHub)](https://github.com/Portkey-AI/portkey-node-sdk)
...
- [ICLR 2025論文解説: RouteLLM — 嗜好データを用いたLLMルーティングによるコスト効率的推論](https://0h-n0.github.io/posts/conf-routellm-iclr2025/) - conference解説
- [論文解説: MeanCache — ユーザー中心セマンティックキャッシュによるLLMサービスコスト削減](https://0h-n0.github.io/posts/paper-2403-02694/) - arxiv解説
- [論文解説: FrugalGPT — LLM APIコストを最大98%削減するカスケード戦略](https://0h-n0.github.io/posts/paper-2310-03744/) - arxiv解説
- [Kong社ベンチマーク解説: AI Gateway性能比較 — Kong・Portkey・LiteLLMのスループット実測](https://0h-n0.github.io/posts/techblog-kong-ai-gateway-benchmark/) - tech_blog解説
- [論文解説: Hybrid LLM — Conformal Predictionによる品質保証付きLLMルーティング](https://0h-n0.github.io/posts/paper-2404-04080/) - arxiv解説
