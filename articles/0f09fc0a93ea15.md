...
| **Claude Opus 4.6** | 200K（1M beta） | 99%+（単一needle）/ 76%（8 needles, MRCR v2） | Prompt Caching | $5.00（$10.00 >200K時） |
| **GPT-4.1 Mini** | 1M | 99%+ | Prompt Caching | $0.40 |
| **Gemini 2.5 Flash** | 1M | 98%+ | Context Caching | $0.30 |

> 単純なNeedle-in-a-Haystack精度だけでモデルを選定するのは危険です。実際のユースケースでは「複数情報の同時検索」「推論を伴う比較」など、より複雑なタスクが求められます。

**注意点**: Claude Opus 4.6の1Mコンテキストはbeta機能で、`anthropic-beta: context-1m-2025-08-07`ヘッダーとTier 4以上のアカウントが必要です。また、コンテキスト長の公称値と実効値には差があり、elvexの調査によると多くのモデルで**実効容量は公称値の60〜70%**程度で急激に性能が低下する傾向が報告されています。

### コンテキスト長の拡大がもたらした変化

1Mトークンは日本語で約50万〜75万文字に相当し、書籍数冊分のテキストを一度に処理できます。この進化により「RAGパイプラインを構築しなくても、ドキュメントを丸ごと投入すれば十分では?」という議論が活発になっています。

しかし、Chroma社が18の最新モデル（GPT-4.1、Claude 4、Gemini 2.5、Qwen3等）を対象に実施した**Context Rot**（コンテキスト腐敗）研究では、**検証対象のすべてのモデルで入力長の増加に伴い性能が劣化する**ことが確認されています。つまり「コンテキストが大きい=常に有利」ではありません。
...
Stanford大学の研究に端を発するこの問題は、2026年の最新モデルでも完全には解消されていません。一般的な傾向として、コンテキストの先頭と末尾の情報は高精度で回収される一方、中間部では精度が低下することが複数の研究で確認されています。
...
関連性のある別の情報（ディストラクター）がコンテキストに含まれると、正答率がさらに低下します。Chroma社の検証では、ディストラクターが4つになると劣化が複合的に進行しました。Chroma社の検証では、Claude系モデルはディストラクター存在時のハルシネーション率が一貫して低い傾向にある一方、GPT系はディストラクター存在時のハルシネーション率が高い傾向が報告されています。
...
本記事で取り上げたトピックについて、1次情報（論文・テックブログ）を深掘りした解説記事を公開しています。

| # | タイトル | ソース |
|---|---------|--------|
| 1 | [LaRA: 大規模RAGベンチマーク（ICML 2025）](https://0h-n0.github.io/posts/paper-2502-09977/) | arXiv 2502.09977 |
| 2 | [Context Rot — 入力トークン増加がLLM性能に与える影響](https://0h-n0.github.io/posts/techblog-context-rot-chroma/) | Chroma Research |
| 3 | [SELF-ROUTE: RAG/ロングコンテキスト動的ルーティング](https://0h-n0.github.io/posts/paper-2407-16833/) | arXiv 2407.16833 |
| 4 | [RAG vs LC-LLMの包括的比較とSelf-Routeハイブリッド手法](https://0h-n0.github.io/posts/paper-2501-12372/) | arXiv 2501.12372 |
| 5 | [Sufficient Context — コンテキスト充足性判定（ICLR 2025）](https://0h-n0.github.io/posts/techblog-sufficient-context-google/) | Google Research |
...
- Context Rotにより、**検証された主要モデルすべてでコンテキスト長の増加に伴い性能が劣化する**。実効容量は公称値の60〜70%程度と見積もるのが現実的
