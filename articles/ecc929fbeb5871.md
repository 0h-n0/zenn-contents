...
- RouteLLMで**APIコスト最大64%削減**を実現する手法
...
マルチLLMルーティングで**APIコスト最大64%削減、品質95%維持**が期待できます。コーディングにClaude Sonnet 4（SWE-bench上位クラス）、大量文書処理にGemini 2.5 Pro（1Mコンテキスト、$1.25/1M入力）を振り分け、**月額$1,750→$632へ64%削減**した試算があります。
...
| GPQA Diamond（推論） | **87.0%** | 84.0% |
| コンテキスト長 | 200K（1M beta） | **1Mトークン** |
| 入力料金（/1Mトークン） | $5.00 | **$1.25** |
| 出力料金（/1Mトークン） | $25.00 | **$10.00** |

Claudeは**コーディング精度**で頭一つ抜けていますが、入力料金はGeminiの約4倍、出力料金は約2.5倍です。Geminiは**大量テキスト処理**で1Mトークンを$1.25で処理でき、コスト効率に優れます。タスクの性質に応じた使い分けでコストを最適化できます。
...
**正規表現ベースの分類はシンプルですが、曖昧なプロンプトでは誤分類が発生しやすい点に注意が必要です。** 本番では次のRouteLLMとの併用が効果的です。

## RouteLLMで自動ルーティングを導入する

RouteLLMはLMSYS（UC Berkeley）がICLR 2025で発表した、嗜好データに基づくルーティングフレームワークです。「強いモデル」と「弱いモデル」の2層構成で、タスク複雑度に応じて自動振り分けします。MT-Benchではコスト85%削減で95%品質維持を達成しています。
...
| 戦略 | 月額コスト | 削減率 |
|------|-----------|--------|
| 全リクエストClaude Opus 4.5 | **$1,750** | — |
| 全リクエストGemini 2.5 Pro | $625 | 64% |
| ルーティング（Sonnet 30%/Gemini Pro 50%/Flash Lite 20%） | **$632** | **64%** |

ルーティング戦略ではClaude Sonnet 4をコーディングに限定し、単純タスクにGemini Flash Liteを使うことで、**Gemini単独と同等コストでコーディング品質を維持**した構成が可能です（Sonnet 4のSWE-bench Verifiedスコアは上位クラス）。
...
- RouteLLMで**品質95%維持・コスト最大64%削減**の自動ルーティングが可能（MT-Benchでは85%削減の報告あり）
- 月間10万リクエスト規模で**月額$1,100以上の削減**（Opus 4.5比）
...
本記事で紹介した技術の詳細については、以下の深掘り記事もご参照ください。

- [論文解説: Mixture-of-Agents — 複数LLMの協調による品質向上](https://0h-n0.github.io/posts/paper-2406-04692/)
- [NVIDIA AI Blueprint: コスト効率の良いLLMルーティング設計](https://0h-n0.github.io/posts/techblog-nvidia-llm-routing-blueprint/)
- [論文解説: REGROUP — 報酬ガイド付きLLMルーティング](https://0h-n0.github.io/posts/paper-2409-03735/)
- [論文解説: One Router, Many Models — Cross-Attentionベースの適応的ルーティング](https://0h-n0.github.io/posts/paper-2506-09157/)
- [AWS Bedrock Intelligent Prompt Routing 技術解説](https://0h-n0.github.io/posts/techblog-aws-bedrock-prompt-routing/)
...
- [Claude API Pricing（公式）](https://platform.claude.com/docs/en/about-claude/pricing)
