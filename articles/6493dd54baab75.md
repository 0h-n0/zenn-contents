...
- log mode → enforce modeの**段階的ポリシー導入**で本番環境の安全性を担保する運用戦略
...
AgentCore Policyとマルチエージェント協調を組み合わせることで、社内申請ワークフローの**自動処理率が約70%に向上**し、承認プロセスの平均所要時間が**3営業日から4時間に短縮された**という事例がAWS公式ブログで報告されています。また、Cedarポリシーによるインフラ層での制御を導入したことで、エージェントの不正なツール呼び出しが**ゼロ件**となり、セキュリティ監査をパスできる水準の統制を実現しています。
...
以下に、社内申請ワークフローで使用するCedarポリシーの例を示します。AWSの公式ドキュメントのパターンを参考に、ワークフロー向けにカスタマイズしています。
...
permit(
    principal is AgentCore::AgentIdentity,
    action in [
    ...
    principal.hasTag("agentRole") &&
    principal.getTag("agentRole") == "validator"
};
...
permit(
    ...
    principal.hasTag("agentRole") &&
    principal.getTag("agentRole") == "approver" &&
...
permit(
    ...
    principal.hasTag("agentRole") &&
    principal.getTag("agentRole") == "notifier"
};
...
Gatewayの設定はAWS CLIまたはSDKで行います。以下は、MCPサーバーをターゲットとして追加する例です。
...
gateway = client.create_gateway(
    ...
    description="社内申請ワークフロー用Gateway",
    protocolType="MCP",
)

gateway_id = gateway["gatewayId"]

# Slack MCP Serverをターゲットとして追加
client.create_gateway_target(
    ...
    targetConfiguration={
        "mcpTargetConfiguration": {
            "mcpEndpoint": {
                "uri": "https://slack-mcp.internal.example.com/mcp",
            },
...
# ツール同期（MCPサーバーからツール一覧を取得）
client.synchronize_gateway_targets(
    gatewayId=gateway_id,
    targetNames=["slack-mcp"],
)
...
**注意点:**
> Gatewayのツール同期は、ターゲット作成時にも暗黙的に実行されます。ただし、MCPサーバー側でツールが追加・変更された場合は、`synchronize_gateway_targets` APIを明示的に呼び出して再同期する必要があります。
...
# ポリシーストアの作成
policy_store = client.create_policy_store(
    gatewayId="workflow-gateway-id",
    description="申請ワークフロー用ポリシーストア",
)

policy_store_id = policy_store["policyStoreId"]
...
print(f"ポリシーストア作成完了: {policy_store_id}")
```

## 段階的にポリシーを本番導入する

AgentCore Policyの強力な機能の一つが、**log mode**と**enforce mode**の切り替えです。本番環境に一気にenforce modeで適用すると、正当なリクエストまでブロックしてしまうリスクがあります。

### 段階的導入のフロー

```mermaid
graph LR
    Dev[開発環境<br/>enforce mode] -->|テスト合格| Staging[ステージング<br/>log mode]
    Staging -->|1-2週間の<br/>ログ分析| Review[ポリシーレビュー]
    Review -->|修正| Staging
    Review -->|問題なし| Prod[本番環境<br/>enforce mode]
...
| ステージング | log | 実トラフィックでの影響分析 | 1-2週間 |
| 本番初期 | enforce（段階的） | クリティカルなポリシーから順次適用 | 1-2週間 |
| 本番安定期 | enforce | 全ポリシー適用・継続監視 | 継続 |

**ハマりポイント:** log modeではポリシー違反をブロックせずCloudWatchに記録するだけなので、**ログを確認せずにenforce modeに切り替えると、意図しないブロックが発生**します。AWS公式ブログでは「log modeで最低1週間のログ分析を推奨」としています。
...
cloudwatch.put_metric_alarm(
    ...
    MetricName="PolicyDeniedRequests",
    Dimensions=[
...
| Policyで正当なリクエストがブロックされる | Cedar条件が厳しすぎる、またはタグ設定の不備 | log modeでログを分析し、条件を調整する |
...
- **log mode → enforce mode**の段階的導入で、本番環境での予期しないブロックを防止しつつ、ポリシーの最適化を進められる
