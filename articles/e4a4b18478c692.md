...
- **権限違反ゼロ**: RBAC準拠のフィルタリングにより未認可ドキュメントのLLM到達を検索段階で遮断
...
社内ナレッジは単一のデータソースに閉じていません。ドキュメントはConfluenceに、設計議論はSlackスレッドに、タスク管理はNotionに分散しています。**検索精度を大きく左右するのは、LLMの性能だけでなくデータソースのカバレッジです。**
...
confluence_store = PGVectorStore(
...
# 本番ではasync withコンテキストマネージャ + await checkpointer.setup()を使用
...
Claude Sonnet 4.6（`claude-sonnet-4-6`、2026年2月リリース）の**adaptive thinking**で、ノードごとに推論深度を調整してコストを最適化できます。
...
2026年2月時点の各種ベンチマークでは、**512トークンの再帰的文字分割**が多くのケースで高い精度を示す傾向にあります。しかし、マルチソースRAGでは各ソースのドキュメント構造に合わせたチャンキングが重要です。
...
本記事の内容に関連する1次情報（論文・テックブログ）の詳細解説記事です。

| タイトル | ソース種別 | 解説記事リンク |
|---------|-----------|--------------|
| DeepRAG: MDP適応型検索判断 | arXiv 2412.10743 | [詳細解説](https://0h-n0.github.io/posts/paper-2412-10743/) |
| NVIDIA HITL AIエージェント（NIM + LangGraph） | 企業テックブログ | [詳細解説](https://0h-n0.github.io/posts/techblog-nvidia-hitl-ai-agent-nim/) |
| Trustworthy RAG Survey: 信頼性6次元フレームワーク | arXiv 2409.11598 | [詳細解説](https://0h-n0.github.io/posts/paper-2409-11598/) |
| Best Practices in RAG: 1,406実験の最適構成 | arXiv 2407.01219 | [詳細解説](https://0h-n0.github.io/posts/paper-2407-01219/) |
| Anthropic Contextual Retrieval: 文脈付与で検索精度改善 | 企業テックブログ | [詳細解説](https://0h-n0.github.io/posts/techblog-anthropic-contextual-retrieval/) |
