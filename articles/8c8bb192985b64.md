...
**Tiger Data社のベンチマーク（50Mベクトル、768次元）で、pgvectorscaleがQdrantの11.4倍のスループット（471 QPS vs 41 QPS、99% recall）を記録。** 一方、p99レイテンシではQdrantが48%優位（38.71ms vs 74.60ms）。最適解はフェーズと要件次第で変わります。
...
|------|-----------|------|----------------|-------------|-------------|
| **pgvector** | PostgreSQL License | C | pgvector + pg_trgm | △（リードレプリカ） | 19,000+ |
| **Qdrant** | Apache 2.0 | Rust | Sparse + Dense | ○（ネイティブ） | 28,000+ |
| **Pinecone** | プロプライエタリ | - | Sparse + Dense | ○（サーバーレス） | - |
| **Milvus** | Apache 2.0 | Go/C++ | BM25 + Dense (v2.5+) | ○（分散設計） | 42,000+ |
| **Weaviate** | BSD-3 | Go | BM25 + Dense | ○（ネイティブ） | 15,000+ |
...
| **p95レイテンシ** | 60.42ms | 36.73ms | Qdrantが**39%優位** |
...
```python
# pgvector_search.py （Python 3.11+, psycopg 3.3+, pgvector 0.4+）
import psycopg
...
```python
# qdrant_search.py （Python 3.11+, qdrant-client 1.16+）
...
**まとめ:**
- pgvectorscaleはQPSで優位（50Mベクトル実測）、テールレイテンシはQdrantが優位
...
- [論文解説: Survey of Vector Database Management Systems](https://0h-n0.github.io/posts/paper-2401-08801/) - arXiv解説
- [論文解説: pgvectorscale — PostgreSQLをAIアプリケーション向けにスケール](https://0h-n0.github.io/posts/paper-2409-02571/) - arXiv解説
- [AWS公式解説: pgvectorインデックス最適化ガイド — IVFFlatとHNSWの深掘り](https://0h-n0.github.io/posts/techblog-aws-pgvector-indexing/) - tech_blog解説
- [論文解説: DiskANN — スケーラブルで高速なフィルタ付き近似最近傍探索](https://0h-n0.github.io/posts/paper-2310-18164/) - arXiv解説
- [Google Research解説: SOAR — ScaNNを加速する直交残差スピリングアルゴリズム](https://0h-n0.github.io/posts/techblog-google-soar-scann/) - tech_blog解説
